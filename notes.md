...
- open issue : to be able to type joint position in yarpmotorgui
- provide documentation on how to calibrate IMU : with robot on pole in home position (i.e. all joints to 0)
  1. Matlab
    ...
    4. look at the IMY scope; the values should be around
      1. 0 for yellow
      2. 0 for blue
      3. 9.8 for red
  ...
  4. add the offsets to the corresponding joints in "CalibraitonDelta"; save and close
  ...
  6. the head uses CAN (not ethernet). The above-mentioned XML file wouls not be loaded properly. Workaround: put the head in initial position required for the IMU (obtained at (\*) as a **workaround for now**
  `~/software/yarp/iCuBContrib/robots/iCubNancy01/calibrators/(some file)`
  "StartupPosition" changed to (\*) in head-calib.xml
  **workaround:** homePoseBalancing.ini is in `.../codyco-superbuild/build/install/share/codyco`. It has been modified according to our measurements: $robot/head-position **was changed** for homePoseYogaApp and joint 0 (original value 0, new value -1.5)
  FIXME: This is not very clear

## How to run yoga demo

Home position Yoga++; head home position also needs to be adjusted

1. `yarpmotorfui --from homePosBalancing.ini`
  -> _menu bar_ global joints commands -> custom positions -> move all parts to YogaApp
2. adjust F/T sensors measurements (adjust offsets) with
  ```
  yarp rpc /wholeBodyDynamics/rpc
  >> calib all 300 # 300 is a timeout in ms
  ```
3. "makumba" to release eventual stress in the legs.
...
4. matlab -> .../codyco-superbuild/main/WBIToobloxControllers/controllers/torqueBalancing
  - torqueBalancingR2015b86.mdl (simulation)
  - README (documentation; outdated) FIXME
  - initTorqueBalancing.m (contains a few more lines of doc)
    - check robot name
    - options to turn on/off scopes
      > FIXME: **Caution:** The scopes should never be turned on when the robot is running
    - `app/robots/iCubNancy01/gains.m` (controller gains, torque saturation value, ...)
    - `app/robots/iCubNancy01/initStateMachine.m` (gains for yoga state machine, overwreites some variables from `gains.m`)
    - `app/robots/iCubNancy01/initRefGen.m` (more parameters)

+ changed hardware joint limites in PID file FIXME: ???
